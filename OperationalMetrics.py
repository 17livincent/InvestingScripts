...
def get_saved_balance_sheet(ticker):
    ...
    with open(
        "data/AlphaVantage/{}/BALANCE_SHEET.json".format(ticker)
    ) as balance_sheet_json:
        balance_sheet = json.load(balance_sheet_json)
        df_data_balance_sheet["date"] = [
            quarterly_report["fiscalDateEnding"]
            for quarterly_report in balance_sheet["quarterlyReports"]
        ]
        df_data_balance_sheet["shareholder_equity"] = [
            quarterly_report["totalShareholderEquity"]
            for quarterly_report in balance_sheet["quarterlyReports"]
        ]
        df_data_balance_sheet["total_debt"] = [
            quarterly_report["shortLongTermDebtTotal"]
            for quarterly_report in balance_sheet["quarterlyReports"]
        ]
        df_data_balance_sheet["cash"] = [
            quarterly_report["cashAndCashEquivalentsAtCarryingValue"]
            for quarterly_report in balance_sheet["quarterlyReports"]
        ]
...
def get_saved_cash_flow(ticker):
    ...
    with open("data/AlphaVantage/{}/CASH_FLOW.json".format(ticker)) as cash_flow_json:
        ...
        df_cash_flow["operating_cash_flow"] = [
            quarterly_report["operatingCashflow"]
            for quarterly_report in cash_flow["quarterlyReports"]
        ]
        df_cash_flow["capex"] = [
            quarterly_report["capitalExpenditures"]
            for quarterly_report in cash_flow["quarterlyReports"]
        ]
...
def get_saved_income_statement(ticker):
    ...
    with open(
        "data/AlphaVantage/{}/INCOME_STATEMENT.json".format(ticker)
    ) as income_statement_json:
        income_statement = json.load(income_statement_json)
        df_data_income_statement["date"] = [
            quarterly_report["fiscalDateEnding"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["total_revenue"] = [
            quarterly_report["totalRevenue"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["cost_of_revenue"] = [
            quarterly_report["costOfRevenue"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["net_income"] = [
            quarterly_report["netIncome"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["operating_income"] = [
            quarterly_report["operatingIncome"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["income_before_tax"] = [
            quarterly_report["incomeBeforeTax"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
        df_data_income_statement["income_tax_expense"] = [
            quarterly_report["incomeTaxExpense"]
            for quarterly_report in income_statement["quarterlyReports"]
        ]
...
def get_saved_fundamentals(ticker):
    ...
    for column_name in df_sorted:
        if column_name != "date":
            df_sorted[column_name] = pd.to_numeric(
                df_sorted[column_name], errors="coerce"
            ).fillna(0)
...
def calculate_operational_metrics(df_fundmentals):
    ...
    df_calculated["debt_to_equity"] = safe_divide(
        df_fundmentals["total_debt"], df_fundmentals["shareholder_equity"]
    )

    # Calculate return-on-equity
    df_calculated["roe"] = safe_divide(
        df_fundmentals["net_income"], df_fundmentals["shareholder_equity"]
    )

    # Calculate gross margin
    df_calculated["gross_margin"] = (
        df_fundmentals["total_revenue"] - df_fundmentals["cost_of_revenue"]
    ) / df_fundmentals["total_revenue"]

    # Calculate operating margin
    df_calculated["operating_margin"] = (
        df_fundmentals["operating_income"] / df_fundmentals["total_revenue"]
    )

    # Calculate net margin
    df_calculated["net_margin"] = (
        df_fundmentals["net_income"] / df_fundmentals["total_revenue"]
    )

    # Calculate effective tax rate
    df_calculated["effective_tax_rate"] = (
        df_fundmentals["income_tax_expense"] / df_fundmentals["income_before_tax"]
    ).clip(lower=0, upper=0.35)
    # Calculate ROIC
    df_calculated["invested_capital"] = (
        df_fundmentals["total_debt"]
        + df_fundmentals["shareholder_equity"]
        - df_fundmentals["cash"]
    )
    df_calculated["invested_capital"] = df_calculated["invested_capital"].where(
        df_calculated["invested_capital"] > 0
    )
    df_calculated["nopat"] = df_fundmentals["operating_income"] * (
        1 - df_calculated["effective_tax_rate"]
    )
    df_calculated["roic"] = df_calculated["nopat"] / df_calculated["invested_capital"]

    # Calculate revenue growth YoY
    df_calculated["revenue_growth_yoy"] = df_fundmentals["total_revenue"].pct_change(
        periods=4
    )

    # Calculate OCF margin
    df_calculated["ocf_margin"] = (
        df_fundmentals["operating_cash_flow"] / df_fundmentals["total_revenue"]
    )
    ...
    df_calculated["average_invested_capital"] = (
        df_calculated["invested_capital"].rolling(4).mean()
    )
