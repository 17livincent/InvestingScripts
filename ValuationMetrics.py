...
def get_shares_outstanding(ticker):
    rows_list = []
    with open(
        "data/AlphaVantage/{}/SHARES_OUTSTANDING.json".format(ticker)
    ) as shares_outstanding_json:
            ...
            rows_list.append(
                {
                    ...
                    "diluted_shares": get_float_or_zero(
                        item["shares_outstanding_diluted"]
                    ),
                }
            )
    ...
    if not rows_list:
        print(
            "WARNING: SHARES_OUTSTANDING data is empty for {}. Falling back to BALANCE_SHEET commonStockSharesOutstanding.".format(
                ticker
            )
        )
    ...
    df_shares_outstanding["date"] = pd.to_datetime(
        df_shares_outstanding["date"]
    ).astype("datetime64[ns]")
    df_shares_outstanding = df_shares_outstanding.sort_values("date").drop_duplicates(
        "date"
    )
...
def get_timeseries_weekly_adjusted(ticker):
    rows_list = []
    with open(
        "data/AlphaVantage/{}/TIME_SERIES_WEEKLY_ADJUSTED.json".format(ticker)
    ) as weekly_stock_json:
            ...
            rows_list.append(
                {
                    ...
                    "adjusted_close": float(
                        weekly_stock["Weekly Adjusted Time Series"][item][
                            "5. adjusted close"
                        ]
                    ),
                    "volume": float(
                        weekly_stock["Weekly Adjusted Time Series"][item]["6. volume"]
                    ),
                }
            )
    df_weekly_stock_close = pd.DataFrame(rows_list)
    df_weekly_stock_close["date"] = pd.to_datetime(
        df_weekly_stock_close["date"]
    ).astype("datetime64[ns]")
...
def calculate_valuation_metrics(
    df_fundamentals, df_weekly_prices, df_shares_outstanding
):
    ...
    # Calculate PE ratio
    df_merged["pe_ttm"] = np.where(
        df_merged["ttm_net_income"] > 0,
    ...
    # Calculate P/FCF
    df_merged["pfcf"] = np.where(
        df_merged["ttm_fcf"] > 0, df_merged["market_cap"] / df_merged["ttm_fcf"], np.nan
    )

    # Calculate enterprise value
    df_merged["enterprise_value"] = (
        df_merged["market_cap"] + df_merged["total_debt"] - df_merged["cash"]
    )

    # Calculate enterprise value to EBIT ratio
    df_merged["ev_ebit"] = np.where(
        df_merged["ttm_operating_income"] > 0,
