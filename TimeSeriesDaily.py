...
def get_time_series_daily_adjusted(ticker_name, minimum_date: datetime):
    ...
    if saved_data is not None and saved_daily_data_is_current(saved_data):
        ...
        data = request_data(
            DAILY_FUNCTION_NAME,
            ticker_name,
            {"outputsize": "full", "datatype": "json", "entitlement": "delayed"},
        )
    ...
    df_time_series_daily["date"] = [
        daily
        for daily in list(data[DAILY_TIME_SERIES_KEY])
        if pd.to_datetime(daily, utc=True) >= minimum_date
    ]

    df_time_series_daily["open"] = [
        float(data[DAILY_TIME_SERIES_KEY][daily_key]["1. open"])
        for daily_key in list(data[DAILY_TIME_SERIES_KEY])
        if pd.to_datetime(daily_key, utc=True) >= minimum_date
    ]

    df_time_series_daily["high"] = [
        float(data[DAILY_TIME_SERIES_KEY][daily_key]["2. high"])
        for daily_key in list(data[DAILY_TIME_SERIES_KEY])
        if pd.to_datetime(daily_key, utc=True) >= minimum_date
    ]

    df_time_series_daily["low"] = [
        float(data[DAILY_TIME_SERIES_KEY][daily_key]["3. low"])
        for daily_key in list(data[DAILY_TIME_SERIES_KEY])
        if pd.to_datetime(daily_key, utc=True) >= minimum_date
    ]

    df_time_series_daily["close"] = [
        float(data[DAILY_TIME_SERIES_KEY][daily_key]["5. adjusted close"])
        for daily_key in list(data[DAILY_TIME_SERIES_KEY])
        if pd.to_datetime(daily_key, utc=True) >= minimum_date
    ]

    df_time_series_daily["date"] = pd.to_datetime(
        df_time_series_daily["date"], utc=True
    )
